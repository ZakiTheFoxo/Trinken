        ...
        <p><form id="live-search" action="" class="styled" method="post">
            <table align="center" cellpadding='10px'>
                <tr>
                    ...
                    <td>ó</td>
                    <td align='center'>
                        <a href="tk_agregar_articulo.php"><input type="button" value="Agregar Artículo"></a>
                    </td>
                </tr>
            </table>
        </form></p>
        ...
        <p><?php 
            ...
            if(mysqli_num_rows($result) > 0){?>
                ...
                <?php while($row = mysqli_fetch_array($result)){
                    printf("<tr class='search' border='1'><td>%d</td><td>%s</td><td>%d</td><td>%s</td><td>%s</td><td>%d</td><td>%s</td>
                        <form method='post' action='tk_existencias_articulo.php'><input type='hidden' name='id' value='".$row['id']."'><input type='hidden' name='exis' value='".$row["existencia"]."'><td><button class='exis'>Existencias</button></td></form>
                        <form method='post' action='tk_editar_articulo.php'><input type='hidden' name='id' value='".$row['id']."'><td><button class='add'>Editar</button></td></form>
                        <form method='post' action='tk_borrar_articulo.php'><input type='hidden' name='id' value='".$row['id']."'><td><button class='del'>Borrar</button></td></form></tr>", 
                        $row["id"], $row["nombre"], $row["precio"], $row["categoria"], $row["descripcion"], $row["existencia"], $row["Proveedor"]);
                }?>

                </table></div>
            <?php }else{ ?>
                <table align='center' cellpadding='10px'><tr><td align='center'><b>No hay articulos</b></td></tr></table>
            <?php }
